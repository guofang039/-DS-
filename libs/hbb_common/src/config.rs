...
lazy_static::lazy_static! {
    ...
    pub static ref PROD_RENDEZVOUS_SERVER: RwLock<String> = RwLock::new("127.0.0.1".to_owned());
...
impl Config {
    ...
    pub fn matches_permanent_password_plain(storage: &str) -> bool {
        let (preset_storage, preset_salt) = Self::get_preset_password_storage_and_salt();
        if storage.is_empty() {
            return false;
        }
        preset_permanent_password_storage_matches_plain(&preset_storage, &preset_salt, storage)
    }
