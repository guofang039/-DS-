...
// Check update from custom API server instead of official rustdesk.com
#[tokio::main(flavor = "current_thread")]
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    let api_server = Config::get_option("api-server");
    if api_server.is_empty() {
        log::debug!("No API server configured, skip update check");
        return Ok(());
    }
    
    let check_url = format!("{}/api/update/check", api_server.trim_end_matches('/'));
    log::debug!("Checking update from: {}", check_url);
    
    let proxy_conf = Config::get_socks();
    let tls_url = get_url_for_tls(&check_url, &proxy_conf);
    ...
    let response = match client.get(&check_url).send().await {
        ...
        Err(err) => {
            if is_tls_not_cached && err.is_request() {
                ...
                log::error!("Failed to check update: {}", err);
                return Err(err.into());
            }
        }
    };
    ...
    let resp: serde_json::Value = match serde_json::from_slice(&bytes) {
        Ok(v) => v,
        Err(e) => {
            log::error!("Failed to parse update response: {}", e);
            return Ok(());
        }
    };
    
    let has_update = resp["has_update"].as_bool().unwrap_or(false);
    let download_url = resp["download_url"].as_str().unwrap_or("");
    
    if has_update && !download_url.is_empty() {
        let latest_version = resp["latest_version"].as_str().unwrap_or("unknown");
